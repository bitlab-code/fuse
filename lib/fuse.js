...
Fuse.prototype.fuse = function (content, matches, relativePath, callback) {
	...
	async.eachSeries(matches, function (match, fn) {
		...
		if(_this.settings.params && typeof _this.settings.params[match.path] == "string") {
			// first, check for placeholders to replace
			var paramContent = _this.settings.params[match.path];
			output = output.replace(match.str, paramContent);
		} else {
			// let's replace placeholders in path
			for(let p in _this.settings.params) {
				match.path = match.path.replace(p, _this.settings.params[p]);
			}
...
exports.fuse = function (inputFile, outputFile, compress, mangle, lint, params) {
	...
	if (params) fuser.set('params', params);
